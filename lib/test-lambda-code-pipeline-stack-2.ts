...
export class TestLambdaPipelineStack2 extends cdk.Stack {
    constructor(scope: Construct, id: string, props?: cdk.StackProps) {
        ...
        const codeCommitRepo = codecommit.Repository.fromRepositoryName(
            this,
            'CodeCommitRepo',
            'test_lambda' // Replace with your CodeCommit repository name
        );
                ...
                phases: {
                    ...
                    pre_build: {
                        commands: [
                            ...
                            'git clone -b $BRANCH_NAME https://********:*******@git-codecommit.eu-north-1.amazonaws.com/v1/repos/test_lambda',
                            'cd test_lambda',
                            'ls -al',
                    ...
                    build: {
                        commands: [
                            ...
                            'docker build -t app .',
                            'docker tag app:latest account_id.dkr.ecr.region.amazonaws.com/test_lambda_ecr_repo:$ECR_TAG',
                        ],
                    },
                    post_build: {
                        commands: [
                            'echo Pushing the Docker image to ECR',
                            'docker push account_id.dkr.ecr.region.amazonaws.com/test_lambda_ecr_repo:$ECR_TAG',
                        ],
                    },
                }
            }),
        ...
        const deployProject = new codebuild.PipelineProject(this, 'DeployProject', {
            ...
            buildSpec: codebuild.BuildSpec.fromObject({
                ...
                phases: {
                  ...
                  --image-uri account_id.dkr.ecr.region.amazonaws.com/test_lambda_ecr_repo:$ECR_TAG`,
                        ],
                    },
                    post_build: {
                        commands: [
        ...
        // Define the Pipeline
        const pipeline = new codepipeline.Pipeline(this, 'CodePipelineTestLambda2', {
            pipelineName: 'CodePipelineTestLambda2',
